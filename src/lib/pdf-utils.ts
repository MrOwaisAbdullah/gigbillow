...
const black = [0, 0, 0];
const gray = [100, 116, 139]; // Muted-foreground
...
export function generateInvoicePdf({ invoice, client, user }: GenerateInvoicePdfProps) {
    const addContent = (doc: jsPDF) => {
        ...
        doc.text(client.name, pageMargin, y + 60 + itemGap);
        
        doc.setFont('helvetica', 'normal');
        doc.text(user.email || '', pageMargin, y + itemGap * 2);
        doc.text(client.email, pageMargin, y + 60 + itemGap * 2);
        ...
        if (invoice.enhancedSummary) {
            ...
            y += (summaryLines.length * (itemGap + 2)) + sectionGap;
        }
